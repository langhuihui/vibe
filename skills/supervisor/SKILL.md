...
> 任何完成声明必须有新鲜的验证证据支持。禁止依赖 Agent 自我报告，禁止"应该"、"可能"等模糊词汇。
...
1. 调用 技术骨干 分析问题（输出到 .vibe/docs/问题分析.md）
   - **必须遵循系统化调试流程**：根因调查 → 模式分析 → 假设测试 → 实现
   - **禁止猜测修复**：未找到根因前不得提出修复方案
   - **3次失败原则**：同一问题修复3次仍失败，必须质疑架构
...
将检测结果记录到 `.vibe/docs/迭代计划.md` 中，确保所有 Agent 知晓：
...
**以下命令会导致整个体系无限等待，必须由调度层防护：**

| 阻塞类型 | 命令示例 | 影响 | 防护措施 |
|----------|----------|------|----------|
| 服务器启动 | `npm run dev`, `cargo run` | 永久阻塞 | 必须后台启动 |
| 监听模式 | `npm test --watch` | 永久阻塞 | 禁止使用 |
| 长时间测试 | `cargo test` | 可能超时 | 必须带超时 |
| 交互式命令 | `npm init` | 等待输入 | 使用 -y 参数 |

## 调度层防护规则

在调用测试专员或开发专员时，**必须在 prompt 中包含以下指令**：

```markdown
## 命令执行要求
1. 执行任何命令前先检测操作系统: `uname -s`
2. 所有测试命令必须带超时: `timeout 300 <command>`
3. 服务器类命令必须后台启动: `nohup <command> &`
4. 禁止使用监听模式: --watch
5. 交互式命令使用自动确认: -y 或 --yes
...
- **参考 command-executor skill（含错误处理规范）处理各类错误**
- **测试阶段可使用 supervisor-fix skill 实现自动测试-修复循环**
- **优先使用并行任务调度提高效率**
- **响应人类的中断请求，优先级高于当前任务**
- **迭代开始时必须检测操作系统类型，传递给所有Agent**
- **所有命令执行必须带超时或后台模式，防止系统中断**
- **参考 command-executor skill 处理跨平台和阻塞问题**
