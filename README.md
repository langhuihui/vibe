...
5. **先思考再编码** — 显式声明假设，不隐藏困惑；存在歧义时呈现多种解释
6. **简洁优先** — 最小代码解决问题，拒绝过度工程；如果 200 行可以变成 50 行，就重写
7. **手术式修改** — 仅修改必须改动的内容；每一行更改都应直接追溯到用户请求
8. **目标驱动执行** — 先定义成功标准（测试），再循环实现；不要告诉 AI 做什么，给它成功标准

### API 设计原则

9. **参数默认即是最优** — 默认值即推荐用法，零配置即可用对
10. **接口命名意图驱动** — 命名表达做什么、解决什么问题，不暴露实现细节
11. **大声报错，且自带解药** — 错误明确可定位，并附带可执行建议或排查指引
